...
```text
"Data science uses data to build systems"
```
...
* "Data" and "data" should be treated as the same word.
...
Now process one word at a time.
...
Is `"data"` already in the dictionary?
...
Continue the same process for every remaining word.

---

### Step 6

After all words are processed, return the dictionary.
...
```mermaid
flowchart TD
    A([Start]) --> B["Read the input sentence"]
    B --> C["Convert sentence to lowercase"]
    C --> D["Split sentence into words"]
    D --> E["Create an empty dictionary (counter)"]
    E --> F["Take the next word"]

    F --> G{"Does the word already exist in the dictionary?"}

    G -- Yes --> H["Increase its count by 1"]
    G -- No --> I["Add the word with count = 1"]

    H --> J{"More words left?"}
    I --> J

    J -- Yes --> F
    J -- No --> K["Return the dictionary"]
    K --> L([End])
```
This is the algorithm. The Python code is simply one way to implement these steps. Thinking in these steps first is exactly how you build algorithmic thinking.
...
Update the base configuration using the override configuration.

* If a value is **not** a dictionary, replace it.
* If a value **is** a dictionary, go inside it and repeat the same process.
...
Take the first key from the override configuration.
...
> Is the value of `"db"` a dictionary?
...
> Does the base configuration also have `"db"`?
...
> Is the base `"db"` also a dictionary?
...
Since both are dictionaries,

**Do not replace the whole `"db"` object.**

Instead,

> Go inside `"db"` and apply the same algorithm again.

---

# Recursive Call

Now compare these two dictionaries.
...
The recursive work is finished.
...
```mermaid
flowchart TD
    A([Start]) --> B["Take one key from override"]
    B --> C{"Is its value a dictionary?"}

    C -- Yes --> D{"Does base have the same key<br/>and is it also a dictionary?"}
    C -- No --> E["Replace value"]

    D -- Yes --> F["Go inside (Recursive Call)"]
    D -- No --> E

    F --> B

    E --> G([Continue with next key / End])
```
...
> **For every key in the override configuration, if both values are dictionaries, go deeper and merge them; otherwise, simply replace the value in the base configuration.**
